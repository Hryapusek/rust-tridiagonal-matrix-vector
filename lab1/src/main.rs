...
fn generate_points(left: f64, right: f64, step_count: usize) -> Vec<f64> {
	let step_size = (right - left) / step_count as f64;
	let mut points: Vec<f64> = Vec::with_capacity(step_count + 1);
...
fn exercise_accuracy_base_example() {
	...
	let ffunc = LambdaFunction::from(|r: f64| -4.0 / r + 6.0 * r);
	...
	let n = 1;
	let original_function = |r: f64| 2.0*r;
	...
	for step_count in [2, 4, 8, 16, 32, 64, 128].iter() {
			...
			let coeff_calculator_v =
					math::coeff_calculator::first_first_calculator::FirstFirstCalculator::new(
							splitter,
							&kfunc,
							&qfunc,
							&ffunc,
			...
			step_count_vec.push(*step_count  as i32);
...
fn exercise_liner_function_base_example() {
	const LEFT: f64 = 1.0;
	const RIGHT: f64 = 10.0;

	// k(r) = 1, q(r) = 0, f(r) = 0 for a simple cylindrical heat conduction case
	let kfunc = LambdaFunction::from(|r: f64| 2.0*r);
	let qfunc = LambdaFunction::from(|_r: f64| 5.0);
	let ffunc = LambdaFunction::from(|r: f64| 50.0*r - 40.0);
	...
	// The 'n' value for cylindrical symmetry (n=1 for 2D axisymmetric cylindrical case)
	let n = 1;
	let original_function = |r: f64| 10.0*r;
...
fn exercise_nonliner_function_base_example() {
	const LEFT: f64 = 1.0;
	const RIGHT: f64 = 10.0;

	// k(r) = 1, q(r) = 0, f(r) = 0 for a simple cylindrical heat conduction case
	let kfunc = LambdaFunction::from(|r: f64| 2.0* r.powf(2.0));
	let qfunc = LambdaFunction::from(|r: f64| 5.0 * r.powf(2.0));
	let ffunc = LambdaFunction::from(|r: f64| 50.0* r.powf(4.0) - 160.0* r.powf(2.0));
	...
	// The 'n' value for cylindrical symmetry (n=1 for 2D axisymmetric cylindrical case)
	let n = 1;
	let original_function = |r: f64| 10.0 * r.powf(2.0);
...
fn main() {
    ...
    let ffunc = LambdaFunction::from(|_r| 0.0);
    ...
    println!("Solution vector v (temperature distribution):\n{}", v);
		...
		// exercise_liner_function_base_example();
		exercise_nonliner_function_base_example();
}
