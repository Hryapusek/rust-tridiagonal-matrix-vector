...
auto DefaultMainMatrixCalculator::calc_a(size_t r_index, size_t t) const -> Number_t
{
  contract(fun)
  {
    precondition(
...
auto DefaultMainMatrixCalculator::calc_b(size_t r_index, size_t t) const -> Number_t
{
  contract(fun)
  {
    precondition(
  ...
  auto up = middle_point(r_points_, r_index + 1) * params_->k(middle_point(r_points_, r_index + 1), 0);
  auto down = calc_h(r_points_, r_index + 1);
  return up / down;
}

/* clang-format off */
auto DefaultMainMatrixCalculator::calc_c(size_t r_index, size_t t) const -> Number_t
{
  contract(fun) {
  ...
  if (r_index == r_points_.size() - 1) {
    return - middle_point(r_points_, r_index) * params_->k(middle_point(r_points_, r_index), 0)
                 / calc_h(r_points_, r_index)
            - params_->hi2
            - params_->q(r_points_[r_index], 0)
...
/* clang-format off */
auto DefaultMainMatrixCalculator::calc_g(size_t r_index, size_t t) const -> Number_t {
  contract(fun) {
    precondition(r_index != 0, "You should not calculate anything for index == 0 - you already have v function");
  };
  if (r_index == 1) {
    return params_->f(r_points_[r_index], 0) +
               middle_point(r_points_, r_index) 
               * params_->k(middle_point(r_points_, r_index), 0)
               / calc_h(r_points_, r_index + 1)
               * params_->v1(0);
  } else if (r_index == r_points_.size() - 1) {
    return params_->f(r_points_[r_index], 0) * calc_cross_h(r_points_, r_index)
            + params_->v2(0);
  } else {
    return params_->f(r_points_[r_index], 0)  * calc_cross_h(r_points_, r_index);
  }
