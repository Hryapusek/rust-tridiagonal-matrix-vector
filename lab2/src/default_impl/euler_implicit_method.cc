...
{
  Eigen::MatrixXd result = Eigen::MatrixXd::Zero(A.rows(), points.size());  // Adjust the columns to match intervals
  ...
  contract(fun)
  {
    ...
    postcondition(result.cols() == points.size(), "result must have the same number of columns as intervals");
  };
